...
For each galaxy in the labels CSV file, this script requests a 256x256 px
...
# Image parameters from the SDSS SkyServer API documentation
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        if (downloaded + failed) % 100 == 0:
            print(f"  Progress: {downloaded + failed}/{len(df)}  downloaded={downloaded}  failed={failed}")
